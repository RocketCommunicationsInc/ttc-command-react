...
const SelectMenuListItem = ({ stepNumber }: PropTypes) => {
  ...
  return (
    <>
      <RuxTableRow>
        <RuxTableCell>{stepNumber}</RuxTableCell>
        <RuxTableCell>
            <div className="pass_select-wrapper">
            <RuxSelect label="Is 'XYZ' Selected?" value={xyz} onRuxchange={(e) => setXyz(e.target.value as string)}>
          ...
          <RuxSelect label="Are you receiving telemetry?" value={receiveTelemetry} onRuxchange={(e) => setReceiveTelemetry(e.target.value as string)}>
            <RuxOption label="yes" value="yes">
              Yes
            </RuxOption>
            <RuxOption label="no" value="no">
